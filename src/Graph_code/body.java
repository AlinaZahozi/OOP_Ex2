...
public class body extends JFrame implements ActionListener {
    ...
    //frame constructor
    public body(DirectedWeightedGraphAlgorithms a) {
        super();
        pa = new board(a.getGraph());
        buildTheMenu();
        this.pack();
    ...
    public void actionPerformed(ActionEvent act) {
        ...
        if (s.equals("show graph G3")) {
            G3 m = new G3(Ex2.take_new_algo_graph("G3.json"));
        } else if (s.equals("show graph G2")) {
            G2 P = new G2(Ex2.take_new_algo_graph("G2.json"));
        } else if (s.equals("show graph G1")) {
            G1 m = new G1(Ex2.take_new_algo_graph("G1.json"));
        } else if (s.equals("graphG1")) {
            graphG1 m = new graphG1(Ex2.take_new_algo_graph("G1.json"));
        } else if (s.equals("graphG2")) {
            graphG2 m = new graphG2(Ex2.take_new_algo_graph("G2.json"));
        } else if (s.equals("graphG3")) {
            graphG3 m = new graphG3(Ex2.take_new_algo_graph("G3.json"));
        }
    }
}