...
public class board extends JPanel {
    ...
    public void main(DirectedWeightedGraph graph) {
        ...
        for (Iterator<NodeData> iterator1 = n; iterator1.hasNext(); ) {
            nextnode = n.next();
            double we = Math.min(smallestvaluex, nextnode.getLocation().x());
            smallestvaluex = we;
            double re = Math.min(smallestvalusey, nextnode.getLocation().y());
            smallestvalusey = re;
            double gy = Math.max(biggestvaluex, nextnode.getLocation().x());
            biggestvaluex = gy;
            double yo = Math.max(biggestvaluey, nextnode.getLocation().y());
            biggestvaluey =yo;
        }
    }
}